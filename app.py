...
# --- POŁĄCZENIE Z GOOGLE SHEETS (SMART) ---
@st.cache_resource
...
def przygotuj_eventy(df_rez, df_sprzet):
    ...
    if not df_rez.empty:
        for _, row in df_rez.iterrows():
            color = "#3788d8"
            title = f"{row['Nazwa']} ({row['Uzytkownik']})"
            if row['ID_Sprzetu'] == 'CALE_LAB':
                color = "#ff9f89"; title = f"⛔ LAB ZAJĘTY: {row['Typ']}"
            events.append({
                "title": title,
                "start": row['Data_Od'],
    ...
    if not df_sprzet.empty:
        ...
        for _, row in wyp.iterrows():
            if row['Data_Zwrotu']:
                # Jeśli mamy datę wypożyczenia w arkuszu, użyj jej jako start. Jeśli nie - użyj "dziś"
                start_date = row.get('Data_Wypozyczenia') if row.get('Data_Wypozyczenia') else datetime.now().strftime('%Y-%m-%d')
                
                events.append({
                    "title": f"WYPOŻYCZONE: {row['Nazwa']} ({row['Uzytkownik']})",
                    "start": start_date,
                    "end": row['Data_Zwrotu'],
                    "color": "#d83737"
                })
...
def sprawdz_dostepnosc(id_sprzetu, data_start, data_koniec, df_rezerwacje):
    ...
    for _, row in lab_rez.iterrows():
        try:
            ...
            if data_start <= r_e and data_koniec >= r_s: return False, f"CAŁE LABO: {row['Uzytkownik']}"
        except: continue
    ...
    if id_sprzetu != 'CALE_LAB':
        ...
        for _, row in sprz.iterrows():
            try:
                r_s = datetime.strptime(str(row['Data_Od']), '%Y-%m-%d').date()
                r_e = datetime.strptime(str(row['Data_Do']), '%Y-%m-%d').date()
                if data_start <= r_e and data_koniec >= r_s: return False, f"{row['Uzytkownik']} ({row['Typ']})"
            except: continue
    return True, ""
...
def main():
    ...
    # --- 5. WYPOŻYCZ (Z DATĄ WYPOŻYCZENIA) ---
            ...
            if st.form_submit_button("Zatwierdź"):
                ...
                for w in wybor:
                    ...
                    if wolne:
                        ...
                        ws_sprzet.update_cell(cell.row, 4, "Wypożyczony") # Status
                        ws_sprzet.update_cell(cell.row, 5, user)         # Kto
                        ws_sprzet.update_cell(cell.row, 6, str(d_zwrot)) # Data zwrotu
                        ws_sprzet.update_cell(cell.row, 7, str(d_wyp))   # Data wypożyczenia (Kolumna G)
                st.cache_data.clear(); st.rerun()
        ...
        if not wyp.empty:
            ...
            if st.button("Zwróć zaznaczone"):
                ...
                for w in wybor:
                    ...
                    ws_sprzet.update_cell(cell.row, 7, "") # Czyścimy datę wypożyczenia
